...
# Список ключевых нишевых триггеров
...
def add_trend(source: str, url: str, title: str, text: str = "", engagement: int = 0):
    ...
    try:
        ...
        log.warning(f"Ошибка сохранения тренда: {e}")
    finally:
        c.close()

def fetch_rss_trends():
    """Сбор свежих трендов и горячих новостей из RSS-лент."""
...
def pitch_trend_to_post(trend_id: int) -> tuple[str, str]:
    """Превращает горячий инфоповод в 2 варианта поста 'на приколе'."""
    c = db()
    ...
    prompt_base = f"Горячая новость/тренд: «{news_title}». Подробности: {news_text[:400]}.\n" \
                  f"Напиши вирусный пост для дейтинг-канала в стиле 'НА ПРИКОЛЕ' с зумерским сленгом, " \
                  f"обстеби ситуацию, дай жизненный вывод и свяжи с отношениями/флиртом."
    
    from news import STYLE, clean_html
    v1 = clean_html(ask(prompt_base + " Сделай упор на иронию и мемный разбор.", system=STYLE, temperature=0.92))
    v2 = clean_html(ask(prompt_base + " Сделай упор на психологию и практический совет.", system=STYLE, temperature=0.94))
    return v1, v2
